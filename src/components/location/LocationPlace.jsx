    ...
    return (
        <Stack direction="column" justifyContent={"right"} alignItems={"center"} margin={"auto"}>
            ...
            <Button sx={{ color: "white", width: "100%" }} onClick={() => setLoc(!loc)}>
                {reduxLocation.location.city ? <Typography color="#ffffff" fontSize={12}>{reduxLocation.location.city}</Typography> :
                    reduxLocation.location.latitude ? <Typography color="#ffffff" fontSize={12}>{reduxLocation.location.latitude}<br />{reduxLocation.location.longitude}</Typography> :
